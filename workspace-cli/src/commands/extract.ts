...
export function extractCommand(program: Command) {
  program
    ...
    .action(async (project) => {
      ...
      verbose(`Analyzing project "${project}" for extraction`, opts);
      ...
      try {
        ...
        let decisionCount = 0;
        const hasDecisions = await fs.pathExists(decisionsFile);
        const hasMemory = await fs.pathExists(memoryFile);

        if (hasDecisions) {
          const content = await fs.readFile(decisionsFile, "utf-8");
          decisionCount = (content.match(/\[ADR-\d+\]/g) || []).length;
        }

        spinner.succeed(chalk.green(`Analysis of ${project} complete.`));

        if (opts.json) {
          output({ project, targetPath, hasDecisions, decisionCount, hasMemory }, opts);
          return;
        }

        const insights: string[] = [];
        if (hasDecisions) {
          insights.push(`${chalk.cyan(String(decisionCount))} architectural decisions found in decisions.md`);
        } else {
